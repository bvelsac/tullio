...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:variable name="fontSize" select="'14.5'"/>
	<xsl:template match="/">
		<xsl:choose>
				...
				<html>
					...
					<body>
						<div style="font-size: {$fontSize};">
							<xsl:apply-templates select="response/resultSet/events//e[@c='true']"
								mode="continu"/>
						</div>
					</body>
				</html>
				...
				<xsl:for-each select="e[@c='true']">
					<xsl:choose>
						<xsl:when test="@lang=$firstLang">
							<xsl:apply-templates select="key('text', @n)"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:apply-templates select="key('trans', @n)"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:for-each>
				<p>&#160;</p>
	...
	<xsl:template match="e" mode="continu">
		<p style="text-indent: -40px;">
			<span style="font-size: 10px; font-family: monospace;">[<xsl:value-of select="@n"/>]
			</span>
		</p>
		<xsl:apply-templates select="key('text', @n)"/>
		<!-- 
  ...
  -->
	<xsl:template match="trans//p[contains(@class, 'title')]//span[string(@title) and
		string(@class)]">
...
<xsl:copy-of select="@class" />
			<xsl:attribute name="style">
				<xsl:choose>
					<xsl:when test="not( @title and string(@class) ) and not(contains(@class,
						'title'))">font-style: italic;</xsl:when>
					<xsl:when test="contains(@class, 'title1')">font-style: normal; font-weight:
						bold; text-align: center; text-transform: uppercase;</xsl:when>
...
<xsl:copy-of select="@class" />
	...
	<xsl:template match="p[@class='title4']">
		<xsl:choose>
			<xsl:when test="preceding-sibling::p[string()][1][not(@class='title4')] or
				preceding-sibling::p[string()][1][@class='title4'][starts-with(.,'betreffende') or
				starts-with(.,'concernant')]">
				<h4 style="font-style: normal; font-weight: bold; margin-left:25px;
					text-indent:-25px;  text-transform:uppercase;   font-size: {$fontSize};">
<xsl:copy-of select="@class" />
					<xsl:apply-templates/>
				</h4>
			</xsl:when>
			<xsl:when test="not(starts-with(., 'concernant') or starts-with(., 'betreffende'))">
				<p style="font-style: normal; font-weight: bold; margin-left:25px;
					text-transform:uppercase;      font-size: {$fontSize}">
<xsl:copy-of select="@class" />
					<xsl:apply-templates/>
				</p>
			</xsl:when>
			<xsl:otherwise>
				<p style="font-style: normal; font-weight: bold; margin-left:25px;     font-size:
					{$fontSize}"><xsl:copy-of select="@class" />
					<xsl:apply-templates/>
				</p>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="span[@class='speaker']">
		...
		<xsl:if test="ancestor::trans and not(contains(.,'voorzitter')  and not (contains(.,
			'College'))  ) and not(contains(.,'résident') and not (contains(., 'réuni')))">
			<xsl:choose>
		...
		<xsl:if test="ancestor::trans and contains(., 'ministre-président') and key('events',
			ancestor::p/@c)/@lang='N'">
			<xsl:text> (en néerlandais)</xsl:text>
		</xsl:if>
		<xsl:text>.-</xsl:text>
	...
	<xsl:template match="p[@class='front']">
		<h1>
			<xsl:attribute name="style">font-style: normal; font-weight: bold; text-align:center;
				font-size: <xsl:value-of select="$fontSize"/>;</xsl:attribute>
<xsl:copy-of select="@class" />
			<xsl:apply-templates/>
		</h1>
	</xsl:template>
...
<xsl:copy-of select="@class" />
			<xsl:attribute name="style">font-style: normal; font-weight: bold; text-align:center;
				text-transform:uppercase; font-size: <xsl:value-of select="$fontSize"
				/>;</xsl:attribute>
			<xsl:apply-templates/>
		</h1>
	</xsl:template>
	<xsl:template match="p[@class='title2']">
		<h2>
<xsl:copy-of select="@class" />
			<xsl:attribute name="style">font-weight: normal; font-style: italic; text-align:center;
				font-size: <xsl:value-of select="$fontSize"/>; </xsl:attribute>
			<xsl:apply-templates/>
		</h2>
	</xsl:template>
