  ...
  this.refreshMetadata = function(oSound) {
    ...
    if (index !== metadata.currentItem) {
      ...
      pl.setPageTitle(metadata[index].title+' | '+metadata.mainTitle);
			if (metadata[index].title == 'STOP') {$("#player").addClass('tooFarGone'); $("#stopPosition").addClass('big'); setTimeout('$("#stopPosition").removeClass("big")', 3000);} 
			else {$("#player").removeClass('tooFarGone')}
