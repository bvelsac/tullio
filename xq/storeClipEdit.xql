...
let $nodes :=  if ($text) then util:eval( $text ) else () 
		...
		let $updateEvents := if ($category = 'orig') then 
			let 	$events-doc := concat("/db/tullio/", $meeting, '/events.xml'),
						...
						$list := for $x in $touch2 
							return $x/@n cast as xs:string,
						$update2 := if ($nodes//events) then 
						...
						$updateLog := if (doc($log-doc)/log) then 
							update insert $logEntry into doc($log-doc)/log
							else xdb:store(concat("/db/tullio/", $meeting), "eventsLog.xml", <log>{$logEntry}</log>)
