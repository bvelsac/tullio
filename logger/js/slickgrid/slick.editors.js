    ...
    var SlickEditor = {
        ...
        YesNoSelectCellEditor : function(args) {
            ...
            this.init();
				...
				LangSelectCellEditor : function(args) {
            var $select;
            var defaultValue;
            var scope = this;

            this.init = function() {
                $select = $("<SELECT tabIndex='0' class='editor-lang'><OPTION value='N'>NL</OPTION><OPTION value='F'>FR</OPTION></SELECT>");
                $select.appendTo(args.container);
                $select.focus();
            };
            ...
            this.loadValue = function(item) {
                $select.val((defaultValue = item[args.column.field]) ? "N" : "F");
                $select.select();
            };
