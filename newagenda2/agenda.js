    ...
    switch (type) {
        ...
        case 'tags':
            ...
            if (stored != undefined && stored != "") {
                ...
                /* create elements for data if they don't exist */
                if ( $(cell).find("TAGS").length == 0 ) {
                	console.log("NO TAGS FOUND");
                	var tagContainer = $("<TAGS />");
                	console.log(persistedData);
                		for (var xc = 0; xc < persistedData.length; xc++) {
                			console.log(persistedData[xc]["name"]);
                			tagContainer.append("<TAG><ID>" + persistedData[xc]["id"] + "</ID> <NAME>" + persistedData[xc]["name"] + "</NAME></TAG>" );
